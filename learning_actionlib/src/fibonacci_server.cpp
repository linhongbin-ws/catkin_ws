...
public:
        ...
        void executeCB(const learning_actionlib::FibonacciGoalConstPtr &goal)
        {
                ...
                ROS_INFO("%s: Executing,creating fibonacci sequence fo order %i with seed %i,%i",action_name_.c_str(),goal->order,feedback_.sequence[0],feedback_.sequence[0],feedback_.sequence[1]);
                ...
                if(success)
                        {
                                ...
                                ROS_INFO("%s:Suceeded",action_name_.c_str());
